...
document.arrive(".css-1t0vdgd", function(viewFromPackage) {
    var html = '';
    getTenant();
    chrome.storage.local.get(["key"]).then((result) => {
        if (result.key) {
                        ...
                        if (entry.tenant == tenant) {
                            ...
                            for (t = 0; t < packages.length; ++t) {
                                let li = '<div id="projTemp" packageName="' + packages[t].packageName + '" class="MuiBox-root css-1r9m1x3" title="' + packages[t].packageName + '"><div class="MuiBox-root css-n1nmwo"><img width=182 alt="' + packages[t].packageName + '" src="' + packages[t].base64Image +
                                    '"></div><div class="MuiBox-root css-flpvhd"><span class="MuiTypography-root MuiTypography-caption css-4wt8i7"> ' + packages[t].packageName + '</span></div></div>'
                                html = html + li;
                            }
                            viewFromPackage.children[0].insertAdjacentHTML("afterend", html);
                        }
                    }
                })
        }
